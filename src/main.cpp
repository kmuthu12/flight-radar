...
void setup()
{
  ...
  // Recovery/setup shortcut:
  // Hold the encoder button during boot for ~1.2 seconds to force the
  // WiFiManager configuration portal, even when saved WiFi credentials exist.
  ...
  {
    const unsigned long setupHoldStarted = millis();
...
void loop()
{
  ...
  if (currentButtonState == LOW &&
      !buttonUsedForZoom &&
      !longPressHandled &&
      millis() - buttonDownAt >= 3000)
  {
    // Reliable setup shortcut from the running radar:
