...
# main.cpp: hold encoder button during boot to force Wi-Fi/config setup portal.
text = read("src/main.cpp")
if "forceSetupPortal" not in text:
    old = r'''  SetLed(255, 255, 0); // WiFi connecting
  WiFiManagerHelpers::ConfigureWiFiManager(wm, tft);
  wm.autoConnect(WiFiManagerHelpers::WiFiManagerName);

  SetLed(0, 255, 0); // Running
'''
    new = r'''  SetLed(255, 255, 0); // WiFi connecting
  WiFiManagerHelpers::ConfigureWiFiManager(wm, tft);

  // Recovery/setup shortcut: hold the encoder button during boot.
  ...
  {
    unsigned long setupHoldStarted = millis();
    tft.fillScreen(lgfx::color888(0, 0, 0));
    tft.setTextColor(lgfx::color888(0, 255, 0));
    tft.drawCentreString("Hold for Setup...", SCREEN_SIZE / 2, SCREEN_SIZE / 2);

    while (digitalRead(ENCODER_SW) == LOW && millis() - setupHoldStarted < 1200)
      delay(10);

    forceSetupPortal = (digitalRead(ENCODER_SW) == LOW &&
                        millis() - setupHoldStarted >= 1200);
  }
  ...
  if (forceSetupPortal)
  {
    tft.fillScreen(lgfx::color888(0, 0, 0));
    tft.setTextColor(lgfx::color888(0, 255, 0));
    tft.drawCentreString("SETUP MODE", SCREEN_SIZE / 2, SCREEN_SIZE / 2 - 12);
    tft.drawCentreString("Connect to ESP32 WiFi", SCREEN_SIZE / 2, SCREEN_SIZE / 2 + 12);
    SetLed(255, 0, 255); // Setup portal active
    wm.startConfigPortal(WiFiManagerHelpers::WiFiManagerName);
  }
  else
  {
    wm.autoConnect(WiFiManagerHelpers::WiFiManagerName);
...
'''
    if old not in text:
        raise SystemExit("Could not locate WiFi connection block in src/main.cpp")
    text = text.replace(old, new, 1)
    write("src/main.cpp", text)
    print("Applied: startup setup portal")
else:
    print("Already applied: startup setup portal")
